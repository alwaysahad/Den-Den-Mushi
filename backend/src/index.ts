    ...
    socket.on("message", (data: RawData) => {
        ...
        try {
            ...
            if (parsed.type === "identify") {
                ...
                if (isNameTaken(nextName)) {
                    if (socket.readyState === WebSocket.OPEN) {
                        socket.send(JSON.stringify({ type: "identify_error", payload: { code: "NAME_TAKEN" } }));
                    }
                    return;
                }
                clientToName.set(socket, nextName);
                if (socket.readyState === WebSocket.OPEN) {
                    socket.send(JSON.stringify({ type: "identity", payload: { name: nextName } }));
                }
                return;
            }
            if (parsed.type === "chat") {
